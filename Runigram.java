...
public class Runigram {
	...
	public static void main(String[] args) {
		...
		*/
		...
		//int n = 10;
		//Color[][] sourceImage = Runigram.read("cake.ppm");
		//Color[][] targetImage = Runigram.read("ironman.ppm");
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for (int i = 0; i < image.length; i ++) {
			for (int j = 0; j < image[i].length; j++){
				vimage[(image.length -1 ) - i][j] = image[i][j];
			}
		}
		return vimage;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		Color[][] scalesImage = new Color[height][width];
		int hSource = image.length;
		int wSource = image[0].length;
		for (int i = 0; i < height; i ++) {
			for (int j = 0; j < width; j ++) {
				scalesImage[i][j] = image[(int)(i * hSource / height)][(int)(j * wSource / width)];
			}
		}
		return scalesImage;
	}
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		...
		Color blandColor = new Color(red, green, blue);
		return blandColor;
	}
	...
	public static Color[][] blend(Color[][] image1, Color[][] image2, double alpha) {
		Color[][] blendImege = new Color[image1.length][image1[0].length];
		for (int i = 0; i < blendImege.length; i ++){
			for (int j = 0; j < blendImege[0].length; j ++){
				blendImege[i][j] = blend(image1[i][j], image2[i][j], alpha);
			}
		}
		return blendImege;
	}
